...
export class HeatmapAgent implements CivicAgent {
  ...
  async execute(context: AgentContext): Promise<AgentContext> {
    ...
    try {
      ...
      // In case of insufficient permissions or offline, log or construct proper error object per guidelines
      const errInfo = {
        error: firestoreError.message || String(firestoreError),
        operationType: 'list',
        path: 'reports'
      };
      console.error('Firestore Error in HeatmapAgent list:', JSON.stringify(errInfo));
      updatedContext.executionLog.push(`⚠️ [Heatmap Agent] Firestore read permission limited. Falling back to dynamic local aggregation...`);
    }
    ...
    try {
      ...
      const errInfo = {
        ...
        operationType: 'write',
        path: `analytics/${analyticsDocId}`
      };
      console.error('Firestore Error in HeatmapAgent write:', JSON.stringify(errInfo));
      updatedContext.executionLog.push(`⚠️ [Heatmap Agent] Could not write to Firestore analytics: database offline or permissions restricted.`);
    }
